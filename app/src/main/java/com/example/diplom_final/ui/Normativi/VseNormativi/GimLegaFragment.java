...
public class GimLegaFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater,
        ...
        View view = inflater.inflate(R.layout.fragment_gimlega, container, false);

        // Инициализация нормативов (ЗАГЛУШКА - используется для updateTargetInfo, если он будет использован)
        initializeNorms();
    ...
    // ЗАГЛУШКА: Заменить на релевантные для жима лежа, если потребуется в будущем
    private void initializeNorms() {
        // Код идентичен DvoboreFragment
        menNorms = new HashMap<>();
        Map<Integer, Double> msNorms = new HashMap<>();
        msNorms.put(60, 155.0); // Пример для жима
    ...
    private void calculateNormative(double weight) {
         // ЗАМЕНЯЕМ ЛОГИКУ НА ЖЕСТКО ЗАДАННУЮ (как в TagaStFragment)
        if (weight <= 0) {
            Toast.makeText(requireContext(), "Введите корректный вес", Toast.LENGTH_SHORT).show();
            resultText.setText(""); // Очищаем результат
            return;
        }

        // Определяем весовую категорию для мужчин (Жим лежа)
        double menCategory;
        ...
        else menCategory = Double.POSITIVE_INFINITY; // "свыше"

        // Определяем весовую категорию для женщин (Жим лежа)
        ...
        else womenCategory = Double.POSITIVE_INFINITY; // "свыше"

        // Находим соответствующие строки в таблицах (используя определенные выше категории)
        ...
        // --- Форматирование вывода мужской категории ---
        String menCategoryLabel;
        if (menCategory == Double.POSITIVE_INFINITY) {
            menCategoryLabel = "свыше 125 кг"; // Жестко задано
        } else {
        ...
        // --- Форматирование вывода женской категории ---
     ...
     private String formatCategoryLabel(double categoryWeight, TableLayout table, boolean isMen) {
        // Код идентичен DvoboreFragment, но с адаптацией текстов
        if (categoryWeight == Double.POSITIVE_INFINITY) {
            // Пытаемся получить текст из последней строки
            ...
            if (lastWeightStr != null && lastWeightStr.toLowerCase().contains("св")) {
                // Извлекаем числовое значение из строки "св. X"
                String numPart = lastWeightStr.replaceAll("[^0-9.,]", "").replace(',', '.');
                return "свыше " + numPart + " кг";
            } else {
                 // Фоллбэк, если последняя строка не содержит 'св'
    ...
    private double findWeightCategory(TableLayout table, double currentWeight) {
        ...
        for (int i = 1; i < table.getChildCount(); i++) {
            ...
            if (view instanceof TableRow) {
                ...
                if (weightCell != null) {
                    ...
                    if (weightText.contains("св.") || weightText.contains("sv.") || weightText.contains("св") || weightText.contains(">")) {
                        try {
                            ...
                            if (currentWeight > threshold) {
                                // Если вес ВЫШЕ порога 'свыше', значит, это категория 'свыше'
                                return Double.POSITIVE_INFINITY;
                            }
                            // Если вес НЕ выше порога, эта строка не подходит как категория 'до'
                            // Но сам порог может быть нужен для сравнения
                            rowWeight = threshold; // Используем порог для поиска
                        } catch (NumberFormatException e) {
                            continue;
                        }
                    } else {
                    ...
                    // Ищем НАИМЕНЬШУЮ категорию, которая БОЛЬШЕ ИЛИ РАВНА текущему весу
                    if (currentWeight <= rowWeight) {
                        double diff = rowWeight - currentWeight;
                        // Выбираем категорию с наименьшей разницей (т.е. ближайшую сверху)
        ...
        // После цикла проверяем особые случаи
        if (categoryWeight == Double.MAX_VALUE && table.getChildCount() > 1) {
            // Если категория не найдена, проверяем, не ниже ли вес минимальной?
            ...
            try {
                ...
                if (currentWeight < firstCategory) {
                    return 0.0; // Вес ниже минимальной
                } else {
                    // Если вес не ниже минимальной, но и не попал ни в одну категорию,
                    // значит он выше последней числовой И не выше порога 'свыше'
                     return Double.POSITIVE_INFINITY; // Возвращаем как 'свыше'
                }
            } catch (NumberFormatException e) {
                return 0.0; // Ошибка парсинга первой категории
            }
        }
    ...
    private TableRow findRowByWeight(TableLayout table, double categoryWeight) {
        ...
        if (categoryWeight == 0.0) return null; // Нет строки для веса ниже минимума
            ...
            if (view instanceof TableRow) {
                ...
                if (weightCell != null) {
                    ...
                    if (categoryWeight == Double.POSITIVE_INFINITY) {
                        ...
                        // Ищем строку с точным числовым значением
                        try {
                             // Убираем 'св.' для парсинга, если ищем НЕ POSITIVE_INFINITY
                            ...
                            continue; // Ошибка парсинга, пропускаем строку
                        }
                    }
                }
            }
        }
        // Фоллбэк для POSITIVE_INFINITY, если точная строка 'св.' не найдена
    ...
    private void setupAgeStatistics() {
        // Код идентичен DvoboreFragment, но с пометкой "Жим лежа"
        if (ageStatsRecyclerView == null) return;
        List<AgeStatItem> ageStats = new ArrayList<>();
        ageStats.add(new AgeStatItem("18-23 (М) - Жим лежа", 160.0, 140.0, 120.0)); // Пример
        ageStats.add(new AgeStatItem("24-39 (М) - Жим лежа", 170.0, 150.0, 130.0)); // Пример
        ageStats.add(new AgeStatItem("40+ (М) - Жим лежа", 165.0, 145.0, 125.0)); // Пример
        ageStats.add(new AgeStatItem("18-23 (Ж) - Жим лежа", 75.0, 65.0, 55.0));  // Пример
        ageStats.add(new AgeStatItem("24-39 (Ж) - Жим лежа", 80.0, 70.0, 60.0));  // Пример
        ageStats.add(new AgeStatItem("40+ (Ж) - Жим лежа", 75.0, 65.0, 55.0));  // Пример
    ...
    private void setupProgressTracking() {
        ...
        progressPrefs = requireContext().getSharedPreferences("gimlega_progress_prefs", Context.MODE_PRIVATE);
    ...
    private void showAddProgressDialog() {
        // Код идентичен DvoboreFragment, НО МЕНЯЕМ ЗАГОЛОВОК
        ...
        builder.setView(dialogView)
               // 4. Изменить заголовок
               .setTitle("Добавить результат (Жим лежа)")
    ...
    private void updateTargetInfo() {
        // Код идентичен DvoboreFragment, НО РАБОТАЕТ С ДАННЫМИ ЖИМА
        ...
        if (menRow != null && menCategoryWeight != 0.0) { // Добавил проверку на 0.0
            ...
            currentRank = "Нет разряда"; // По умолчанию
            nextRank = "III"; // По умолчанию
            nextTargetWeight = norms[0];
             ...
             for (int i = norms.length - 1; i >= 0; i--) {
                 if (norms[i] > 0 && lastResult >= norms[i]) {
                    ...
                    break; // Нашли текущий разряд, выходим
                 }
             }
             // Если ни один норматив не выполнен
